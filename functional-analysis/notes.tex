...
\begin{definition}[Definition and proposition]
	...
	\begin{itemize}
		...
		\itme $\sum_{n=1}^\infty x_n$ is \emph{absolutely convergent}\index{Absolutely convergent} if $\sum_{n=1}^\infty \Norm{x_n}$ converges [$\iff \left(\sum_{n=1}^m \Norm{x_n}\right)_m$ is bounded]
	\end{itemize}
...
\begin{proof}
	\begin{description}
		...
		\item[$\impliedby$]
			...
			\[ \sum_{k=1}^n y_k = x_{n_{m+1}} - x_{n_1} \implies x_n_{m+1} \to y - x_{n_1} \text{ as } n \to \infty \]
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[4.] Take $([x_n])_n$ to be a sequence in $X/Y$ and suppose that $\sum_{i=1}^\infty \Norm{[x_n]}_{X/Y} < \infty$. If we can show that $\exists [x] \in X/Y$ such that $\sum_{i=1}^\infty [x_n] = [x]$, then by Proposition~\ref{proposition:2.2}, $X/Y$ is complete.

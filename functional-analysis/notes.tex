		...
		\item[Well-defined]
			...
			\begin{description}
				\item[Case $q < \infty$:] Define 
					...
					Further:
					\begin{align} % Slightly overful hbox
...
\begin{definition}
	...
	We say $(x^*_n)_n$ \emph{weak*-converges}\index{Weak* convergence} to $x^*$ and write $x^*_n \weaksc x^*$ if $x^*_n(x) \to x^*(x) \forall x \in X$
\end{definition}
...
\begin{definition}
	...
	\]
	$U^\bot, V_\bot$ are called \emph{annihilators}\index{Annihilators} of $U$ and $V$.
\end{definition}
...
\begin{proof}
	Consider $G \times L$ with $\Norm{(x, y)}_{G \times L} \coloneqq \Norm x + \Norm y$. Define $T: G \times L \to G + L$ with $(x, y) \mapsto x + y$.
	Thus $T$ is linear, surjective. By the open mapping theorem, $\exists \varepsilon > 0: B_\varepsilon(0) \subset T(B_1(0))$.
	\begin{align*}
		\implies \forall z \in G + L: &\Norm z < \varepsilon \implies z = x + y \text{ with } \Norm x + \Norm y \leq 1 \\
		\implies \forall z \in G + L: & \frac{\varepsilon z}{2 \Norm{z}} \in B_\varepsilon(0) \implies \frac{\varepsilon z}{2 \Norm z} = x + y \text{ with } \Norm x + \Norm y \leq 1 \\
		\implies z &= \frac{x \Norm z 2}{\varepsilon} + \frac{y \Norm z 2}{\varepsilon} = \hat x + \hat y & \text{ with } \Norm x + \Norm y \leq 1
	\end{align*}
	and 
	\[ \Norm{\hat x} = \frac{\Norm x \Norm z 2}{\varepsilon} \leq \frac2\varepsilon \Norm z \qquad \Norm{\hat y} = \frac{\Norm y \Norm z 2}{\varepsilon} \leq \frac2\varepsilon \Norm z \]
\end{proof}

\begin{proposition}
	\label{proposition:7.3}
	Let $X$ be normed, $P: X \to X$ is called \emph{projection}\index{Projection} if $P \circ P = P$. $P$ is called linear and continuous projection if it is linear and continuous. $P$ is called \emph{projection}\index{Projection} to $U \subset X$ if $P(X) \subset U$. Also, we write $X = A \oplus B$ for $A, B \subset X$ subspaces if $X = A + B$ and $A \cap B = \Set{0}$.
	\[ \implies \forall x \in X \exists! a \in A, b \in B: x = a + b \]
	If P is a continuous, linear projection, then
	\begin{enumerate}
		\item $P = 0$ on $\Norm P \geq 1$
		\item $\ke(P)$ and $\operatorname{range}(P)$ are closed
		\item $X = \ke(P) \oplus \operatorname{range}(P)$ [\enquote{projection yields decomposition of $X$}]
	\end{enumerate}
\end{proposition}
...
\begin{definition}
	...
	We say, $U$ has finite \emph{co-dimension}\index{Co-dimension} if $\exists V \subseteq X: \dim(V) < \infty$ and $U + V = X$.
\end{definition}
	...
	\begin{enumerate}
		...
		\item \begin{description}
			...
			\item[$\supseteq$] Let $\hat u \in U^\bot_\bot$ (since $U^\bot_\bot$) is closed.
				Assume that $\hat u \not\in \overline U$.
				By Hahn-Banach,
				\[ \exists x^* \in X^*: \Re(x^*(u)) < \alpha < \Re(x^*(\hat u)) \forall u \in U \] % TODO or \forall u \in \overline U
...
\begin{definition}[Definition and proposition]
	...
	We define a \emph{dual operator}\index{Dual operator} or \emph{adjoint operator}\index{Adjoint operator} to $T$ as $T^*: Y^* \to X^*$
...
\begin{Example}
	...
	\end{align*}
	\[ \implies \tilde y^* \coloneqq (\tilde y_i) \] % TODO: verify, chris' notes unreadable
	$Ty^* = \tilde y^*$
...
\begin{definition}
	...
	Let $H$ be Hilbert. $S \subset H$ is called \emph{orthonormal system}\index{Orthonormal system} if
	\begin{enumerate}
		\item $\forall e \in S: \Norm e = 1$
		\item $\forall e, f \in S: \IP{e}{f}_H = 0$
	\end{enumerate}
	An orthonormal system $T$ is called \emph{orthonormal basis}\index{Orthonormal basis} [or complete \emph{orthonormal system}]
...
\begin{proposition}[Bessel inequality]\index{Bessel inequality}
	...
	\begin{enumerate}
		...
		\item TFAE:
			\begin{enumerate}
				...
				\item $\Norm{x}^2 = \sum_{e \in S} \Abs{\IP xe}^2$ (Parseval's identity)\index{Parseval's identity}
			\end{enumerate}
	\end{enumerate}
\end{proposition}

...
\begin{proposition}
	...
	Let $X$ be normed, $P: X \to X$ is called \emph{projection}\index{Projection} if $P \circ P = P$. $P$ is called linear and continuous projection if it is linear and continuous. $P$ is called projection to $U \subset X$ if $P(X) \subset U$. Also, we write $X = A \oplus B$ for $A, B \subset X$ subspaces if $X = A + B$ and $A \cap B = \Set{0}$.

...
\begin{proof}[Proof of theorem~\ref{7.14}]
	Statement (1) equals statement (3).
	TODO
	\begin{description}
		\item[$\supset$] 
		...
		\item[Linear] Continuous.
			...
			By assumption, $\operatorname{range}(S^*)$ is closed. Also, $S^*$ is injective. Since $\ke(S^*) = \operatorname{range}(S)^\bot = \Set{0}$ by Proposition~\ref{proposition:7.13}.
...
\begin{proof}
	This is a consequence of Theorem~\ref{theorem:7.14} and Proposition~\ref{proposition:7.13} and of the fact that $\Norm{T} = \Norm{T^*}$ and $\Norm{T} = 1 \iff T$ is an isometry (proof is left as an exercise to the reader).
\end{proof}
...
\begin{Remark}[Example]
	...
	$L^2$ is a typical example of an inner product space.
	TODO
\end{Remark}
...
\begin{proof}
	\begin{description}
		...
		\item[$\impliedby$] Define $\IP{\cdot}{\cdot}$ as in Proposition~\ref{proposition:8.5} + computation (compare with the book by Werner)
	\end{description}
\end{proof}

...
The halfspace $\SetDef{y}{y^t b \geq 0}$ contains a \emph{polyhedral cone}\index{Polyhedral cone} $\SetDef{y}{y^t A \geq 0}$
...
\begin{Remark}[Remark about (2)]
	...
	There is only one direction in (2). % TODO what is this supposed to mean?
\end{Remark}

Our goal is to establish a result about the existence of a minimum.

\begin{Definition}
	Let $f: \mathbb R^n \to \mathbb R$. A \emph{level set}\index{Level set}\dt{Niveaumenge} is defined by
...
\begin{lemma}
	...
	Then $d$ is the \emph{direction of descent}\index{Direction of descent} of $f$ in $x$.
\end{lemma}
...
This actually gives a line, the \emph{Armijo-Goldstein line}\index{Armijo-Goldstein line}.
...
One variation of the WP rule is the \emph{strict WP rule}\index{Strict Wolfe-Powell rule}.
...
% TODO drawing, Chris does not have it
...
\begin{Definition}[Q-convergence]
	\begin{enumerate}
		...
		\item The value $\inf\SetDef{p \in [1, \infty)}{Q_p(\Set{x^{(k)}}) = \infty}$ is called \emph{Q-convergence order}\index{Q-convergence order} of $\Set{x^{(k)}}$.
	\end{enumerate}
\end{Definition}
...
\begin{Remark}[Concluding remarks]
	\begin{enumerate}
		...
		\item As generalization of the Newton's method, so-called \emph{inexact} Newton's methods were proposed and researched. Instead of satisfying the Newton equation $\nabla^2 f(x^{(k)}) d^{(k)} = -\nabla f(x^{(k)})$ accurately in every Newton step [difficult anyways, because of precision and computational requirements], approximation solutions are considered acceptable. To measure inaccuracy, the \emph{relative error}\index{Relative error in Newton's method}
		\[ \frac{\Norm{\nabla^2 f(x^{(k)}) d + \nabla f(x^{(k)})}}{\Norm{\nabla f(x^{(k)})}} \]
...
This defines the Quasi-Newton condition (sometimes called \emph{secant method}\index{Secant method}).
...
\begin{Remark}[Motivation of QNC]
	\begin{enumerate}
		...
		\item By $\nabla f(x^{(k+1)}) - \nabla f(x^{(k)}) = \int_0^1 \left(\nabla^2 f(x^{(k)} + t \left(x^{(k+1)} - x^{(k)}\right)\right) \, dt \left(x^{(k+1)} - x^{(k)}\right)$
			where $M(x^{(k)}, x^{(k+1)}) \coloneqq \left(\nabla^2 f(x^{(k)} + t \left(x^{(k+1)} - x^{(k)}\right)\right)$ is called \emph{mean value matrix}\index{Mean value matrix} we get that $M(x^{(k)}, x^{(k+1)})$ satisfies the QNC. In the special case of a quadratic function $f(x) = \frac12 x^t Qx + c^t x + \gamma$, the Hessian matrix satisfies the QNC. In general, the Hessian matrix does \emph{not} satisfy the QNC.
	\end{enumerate}
\end{Remark}
...
% Inspection, 1st exam: 2019/06/25 18:00
% Questions? 2019/06/25 16:15–18:00
...
Sometimes the second variant is called \emph{inverse BFGS formula}\index{Inverse BFGS formula}.
...
\begin{theorem}
	...
	so-called \emph{inverse form of the BFGS-formula}\index{BFGS-formula}.
\end{theorem}

...
\begin{proof}[Proof of Theorem~\ref{theorem:5.3.3}]
	The global minimum must lie in $\mathcal L(x^{(0)})$, hence $f$ has exactly one global minimum (implication follows from a result in chapter 2, the preconditions are satisfied here).
...
\begin{Remark}[About the choice of $t_j$ in phase 2]
	...
	\[ q_0(a_j) = \varphi(a_j) \qquad q_j'(a_j) = \varphi'(a_j) \qquad q_j(b_j) = \varphi(b_j) \]
\end{Remark}
...
\begin{example}
	\begin{itemize}
		...
		\item $x^{(k)} = a^{2k}$ with $a \in (0, 1)$. Consider $p = 2$.

...
\title{
  ...
  \small{Based on a lecture by Prof. Eranda Dragoti-Çela} \\
  \small{Extended during a lecture by Prof. Bettina Klinz}
}
\date{\today}
\author{Lukas Prokop}
...
\begin{algorithmic}[1]
  ...
  \For{$e = (v, w) \in E(G_i)$} % TODO: verify correctness
    ...
    \Statex
      \hskip\algorithmicindent $v' = C$ if $v \in V(C)$ for $C \in \mathcal{C}$ and
      $\qquad v' = v$ if $v \notin \bigcup_{C \in \mathcal{C}} V(C)$ and
    \Statex
      \hskip\algorithmicindent $w' = C$ if $w \in V(C)$ for $C \in \mathcal{C}$ and
  ...
  \For{$e = (v, w) \in E(G_i)$ with $e' = (v', w') \in E(G_{i+1})$} % TODO: verify correctness
    ...
    \Statex
      \hskip\algorithmicindent if $w' \in C \in \mathcal{C}$ where $\alpha(e, C) \in \delta^-_C(w)$ and $e_C$ is the cheapest edge of $C$.
  \EndFor
  \State Set $i := i + 1$
  \Goto{eba-step-2}
...
\begin{definition}
  Let $G = (V, W)$ be a digraph with $c: E \rightarrow \mathbb{R}$.
\end{definition}
...
\begin{definition} \emph{(dt. ``Inkrementnetzwerk'', residual network)}
  ...
  \] \[
      ...
      \end{array}\right.
  \]
  $(G_f, u_f, s, t)$ is then called \enquote{residual network}.
\end{definition}
...
\begin{definition}
  $s$-$t$-flow $f$ is called \emph{blocking}, if graph $(V(G), \set{e \in E(G_F^L): f(e) < u(e)})$ does not contain a \gath st.
\end{definition}
...
%
\index{b-flow}
\index{offering vertex}
\index{demanding vertex}
...
\subsection{Initial flow for successively shortest path algorithm}
\begin{itemize}
...
\index{$q_G(X)$}
Let $q_G(X)$ be the number of odd (in terms of number of vertices) connected components in $G \setminus X$.
...
\begin{proof}
  ...
  \begin{observation}[$\card{V(G)}$ is even]
    If $\card{V(G)}$ is odd:
    Let $X = \emptyset$ and $q_G(\emptyset) =$ number of connected, odd components in $G$.
    \[
  ...
  \begin{observation}
    Every odd connected component of $G-X$ is factor-critical.
...
\begin{align*}
  G(E, \mathcal{F}, c) &= c(G_n) = \sum^n_{j=1} \underbrace{\left(\card{G_j} - \card{G_{j-1}}\right)}_{1 \text{ or } 0 \text{ depending on } e_j} c(e_j) = \sum_{e_j \in G_n} c(e_j) \\
    ...
    &\geq \card{O_1} (c(1) - c(2)) + \ldots + \card{O_n} c(e_n) = \card{O_1} c(e_1) \\
    &+ (\card{O_2} - \card{O_1}) c(e_2) + \ldots + (\card{O_n} - \card{O_{n-1}}) c(e_n) \\
...
\textbf{Remark.}
  ...
  Hence the maximization problem handled by the GREEDY-IN algorithm is equivalent to
  \[ \min\set{\overline{c}(B) = \sum_{e \in B}\overline{c}(e): B \text{ is base of } (E, \mathcal{F})} \]
  where $\overline{c}(e) = M - c(e)$ with $M = \max_{e \in E} \card{c(e)} + 1$.
...
\textbf{Example.}
  ...
  \[
    P(M(G)) = \set{x \in \mathbb{R}^{\set{E(G)}}: x \geq 0,
      \sum_{e \in A} x_e \leq n - 1 \fall A \subsetneqq E(G),
      \sum_{e \in A} x_e = n - 1, A = E(G)}
  \]
  where the first sum is independent of components of $M(G)$ are forests

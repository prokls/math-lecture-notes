...
\index{Stabilized Newton process}
...
\begin{example}[Newton process to compute the inverse]
  ...
  By the Newton process the following iteration follows:
...
\paragraph{Simplified and stabilized Newton process}

Stabilized Newton process is usually implemented by Line Search and thus Line Search might be a better keyword for it.
...
In the simplified Newton process, we choose $c \approx z$ and iterate $f'(c)(x^{(k+1)} - x^{(k)}) = -f(x^{(k)})$ and $k = 0, 1, 2$.
For example, $c = x^{(0)}$.
\begin{description}
...
\begin{itemize}
  ...
  \item Modify the simplified Newton process such that for all $l$ steps a new computation of $f'(c)$ happens
\end{itemize}

In the stabilized Newton process we modify the Newton iteration according $x^{(k+1)} = x^{(k)} - \lambda_k f'(x^{(k)})^{-1} f(x^{(k)})$ with a stabilization factor $\lambda \in (0, 1]$.

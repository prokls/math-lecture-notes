...
\begin{theorem}
  ...
  Then there exists exactly one $x \in \mathbb R^n$ such that $Ax = b$.
  \[
    ...
    \end{bmatrix}
    \qquad \text{\enquote{Cramer's Rule}}
  \]
\end{theorem}
...
\[ \Norm{b - Ax} \to \operatorname{min} \qquad \text{ where } b = \begin{pmatrix} b_1 \\ \vdots \\ b_m \end{pmatrix}, x = \begin{pmatrix} x_1 \\ \vdots \\ x_n \end{pmatrix}, A = (a_{ij}) \in \mathbb R^{m \times n} \text{ with } a_{ij} \coloneqq a_j(t_i) \]

...
\begin{proof}
  ...
  \begin{theorem}[Gershgorin theorem]
...
\centering{trapezoidal rule $\equiv \hat{I}(f)$}
...
\subsection{Integration over default triangle}
...
\begin{table}[t]
  \begin{center}
    ...
    \hline
      $(\frac 13, \frac 13, \frac 13)$ & $\frac12$ & 1 \\
...
\subsection{Higher dimensions integrals}
...
For large $d$ the error margin decreases. To halve the approximation error, we need to apply $2^{\frac d2}$ more functionevaluations (\emph{curse of dimensionality}).

\subsection{Monte-Carlo integration}

In the following, we denote the Lebesgue measure on $\mathbb R^d$ with $\gamma$.
For $Q = \prod_{i=1}^d [a_i, b_i]$ we have $\gamma(Q) = \prod_{i=1}^{d} (b_i - a_i)$.

...
\begin{remark}
  ...
  It is feasible, that $f$ in $x$ has a limit, $x \in A$ and $c = \lim_{\xi\to x} f(\xi) \neq f(x)$.
\end{remark}
...
\begin{enumerate}
  \item Let $x \in (a, b)$. Let $\varepsilon$ as above such that \eqref{neghypo} holds on every interval $[a_n, b_n]$.
    ...
    (feasible because $([a_n, b_n])_{n \in \mathbb N}$ gives nested intervals tightening on $x$).
...
\begin{corollary}[Sufficient conditions for existence of extreme values]
  ...
  \[ f^{(1)}(x_0) = f^{(2)}(x_0) = \dots = f^{(n)}(x_0) = 0 \] % TODO I think he gave some examples here - content missing?!
...
\begin{example}[Counterexample]
  ...
  So not every infinitely often differentiable function must not equate with its Taylor series. % TODO: double negation? wrong or confusing.
\end{example}
...
\begin{example}
  ...
  A norm on $\mathbb R^{n \times m}$ is called \emph{matrix norm} ($\mathbb C^{n \times m}$).
\end{example}
...
\begin{remark}
  ...
  \[ \iff \frac{\card{f_n(x) - f_k(x) - a_k(x - x_0)}}{\norm{x - x_0}} \xrightarrow{x \to x_0} 0 \] % TODO criterion for k missing
...
\begin{remark}
  ...
  Consider $T_s = \setdef{x \in D}{f(x) = s}$. Compare with Figure~\ref{img:niveau}.
  ...
  $I$ is an interval. $\Gamma_s = \setdef{\gamma_s(t)}{t \in I}$. We assume, that $\gamma_s$ is regular, hence $\gamma$ is differentiable
  ...
  It holds that
  \[ \angle{\nabla f(\gamma_s(t)), \gamma'_s(t)} = \angle{\nabla f(x), v} = 0 \]
  \[ f(\underbrace{\gamma_s(t)}_{\in \Gamma_s}) = s \qquad \text{ constant} \]

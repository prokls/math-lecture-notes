...
\begin{lemma} % Lemma 8
  ...
  Then $Df(x_0) = [0, \dots, 0]$ or $\nabla f(x_0) = \begin{bmatrix} 0 & 0 & \vdots & 0 \end{bmatrix}^T$.
\end{lemma}
...
\begin{definition}
  Let $[\gamma]_{\sim_r}$ be an equivalence class of $\gamma$ in regards of $\sim_r$. A size or property, which only depends on $[\gamma]_{\sim_r}$ but not on a specific representative $\gamma$ is called \emph{geometrical quantity}.
\end{definition}

...
\begin{definition} % Definition 7
  ...
  Or equivalently for strict maxima:
...
\begin{lemma} % Lemma 9
  ...
  Furthermore let $g: D \to g(D)$ bijective and $Dg(x)$ is regular for all $x \in D$.
...
\begin{proof}
  ...
  \begin{figure}[t]
    \begin{center}
      ...
      \caption{TODO put notes here that should be in the image (see Ring notes)}
      \label{img:x1x2}
    \end{center}
  \end{figure}
  ...
  \begin{align*}
	\norm{\varphi_y(x_2) - \varphi_y(x_1)}
	  ...
	  &\leq \int_0^1 \norm{D\varphi_Y(l_{x_1 x_2}(t))} \cdot \norm{(x_2 - x_1)} \, dt \\ % TODO underbraces missing
	  &\leq (*)
  \end{align*}
  \[ D \varphi_Y(x) = I - DF(x) \]
...
\begin{theorem}[Implicit function theorem] % Satz 4
  ...
  Notation: $\vectwo xy \in U; x \in \mathbb R^n, y \in \mathbb R^m$
  \[ f(\vectwo xy) = f(x, y) \]
